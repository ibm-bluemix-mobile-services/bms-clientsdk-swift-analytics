...
class LocationDelegateTests: XCTestCase {
    ...
    func testRecordMetadataForUniqueEvent() {
        ...
        locationDelegate.recordMetadata(LocationDelegateTests.sampleMetadata, locationManager: CLLocationManager())
        self.waitForExpectations(timeout: 0.1, handler: nil)
    }
...
class LocationDelegateTests: XCTestCase {
    ...
    static let appSessionID = "5F9468D4-5AB2-4739-8F1E-B06318DB43B6"
    static let originalTimestamp = NSNumber(longLong: 1000000000)
    static let futureTimestamp = NSNumber(longLong: 2000000000)
    static let newLocation = CLLocation(latitude: 30.4, longitude: -97.715)
    
    static let sampleMetadata: [String: AnyObject] = ["$category": "userSwitch", "$userID": "Anthony", "$latitude": newLocation.coordinate.latitude, "$longitude": newLocation.coordinate.longitude, "$timestamp": LocationDelegateTests.originalTimestamp, "$appSessionID": LocationDelegateTests.appSessionID]
    
    static let sampleMetadataWithoutLocation: [String: AnyObject] = ["$category": "userSwitch", "$userID": "Anthony", "$timestamp": LocationDelegateTests.originalTimestamp, "$appSessionID": LocationDelegateTests.appSessionID]
    ...
    class BMSLoggerMock: BMSLogger {
        ...
        // The LocationDelegate's main function is to log the analytics metadata
        // Here, we intercept the log to check if LocationDelegate successfully logged the sampleMetadata
        override func logToFile(message logMessage: String, level: LogLevel, loggerName: String, calledFile: String, calledFunction: String, calledLineNumber: Int, additionalMetadata: [String : AnyObject]?) {
            
            // Check if the logged metadata matches the expected sampleMetadata
            if additionalMetadata != nil && additionalMetadata?[Constants.Metadata.Analytics.sessionId] as? NSString == LocationDelegateTests.appSessionID {
    ...
    func testDidUpdateLocationsWithMetadata() {
        ...
        let expectation = self.expectationWithDescription("didUpdateLocations should successfully log the metadata with location information")
    ...
    func testDidUpdateLocationsWithoutLocation() {
        ...
        let expectation = self.expectationWithDescription("didUpdateLocations should successfully log the metadata without location information")
    ...
    func testDidFailWithErrorWithMetadata() {
        ...
        let expectation = self.expectationWithDescription("didFailWithError should successfully log the metadata without location information")
        
        Logger.delegate = BMSLoggerMock(expectation: expectation, hasMetadata: true, hasLocation: false)
        
        let locationDelegate = LocationDelegate()
        locationDelegate.analyticsMetadata = LocationDelegateTests.sampleMetadataWithoutLocation
        locationDelegate.locationManager(CLLocationManager(), didFailWithError: NSError(domain: "", code: 0, userInfo: nil))
    ...
    func testRecordMetadataForUniqueEvent() {
        ...
        let expectation = self.expectationWithDescription("recordMetadata should successfully log the metadata with location information if this is a new event (i.e. different timestamp from previous event)")
        
        Logger.delegate = BMSLoggerMock(expectation: expectation, hasMetadata: true, hasLocation: true)
        
        let locationDelegate = LocationDelegate()
        // Simulate a new analytics event with a future timestamp
        locationDelegate.previousTimestamp = LocationDelegateTests.futureTimestamp.longLongValue
        locationDelegate.recordMetadata(LocationDelegateTests.sampleMetadata, locationManager: CLLocationManager())
        self.waitForExpectationsWithTimeout(0.1, handler: nil)
    }
    ...
    func testRecordMetadataForRepeatEvent() {
        ...
        let locationDelegate = LocationDelegate()
        // Simulate a duplicate analytics event with the same timestamp as a previously recorded event
        locationDelegate.previousTimestamp = LocationDelegateTests.originalTimestamp.longLongValue
        locationDelegate.recordMetadata(LocationDelegateTests.sampleMetadata, locationManager: CLLocationManager())
